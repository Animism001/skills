---
name: history-story
description: 历史故事创作技能。当用户要求创作历史故事、了解"历史上的今天"、将历史事件改编为故事、或提到"历史故事"、"今天的历史"、"历史上的今天"、"写一个历史故事"时使用此技能。即使用户只是说"今天历史上发生了什么"或"给我讲一个历史故事"，也应触发。此技能会先调研历史上的今天发生的事件，再深入调研具体史实，由选题组选出最有故事价值的事件并提出创作要求，用D.E.S.I.R.E. Matrix分析事件与人物，生成人物设定和故事大纲，最后交由Storyteller子代理创作引人入胜的历史故事。
---
...
| **PitchEditor** 选题编辑 | `agents/pitch-editor.md` | 选题决策、提出创作要求 |
...
### Phase 3: 选题与创作要求（PitchEditor）

阅读 `agents/pitch-editor.md` 获取详细指令。

选题编辑通读全部调研结果后，做出选题决策：

1. **通读全部 research.md**，对当日所有历史事件形成全局理解
...
3. **为每个入选事件提出创作要求**：
   ...
   - 字数建议
   - 目标读者画像
4. 保存到 `./history/{MM}/{DD}/pitch.md`

**选题逻辑**：不是选"最重要"的事件，而是选"最适合讲成故事"的事件。一个影响深远但缺乏人物张力的条约签署，不如一个影响较小但人物命运跌宕的小事件。

### Phase 4: D.E.S.I.R.E. Matrix 分析（Analyst，并行入选事件）

阅读 `agents/analyst.md` 获取详细指令。

仅对 pitch.md 中入选的事件进行分析，两层分析：
...
整合所有素材设计故事大纲：
- 读取 pitch.md（创作要求）、research.md、matrix.md、characters/、relationships.md
- 设计场景序列、叙事视角、情感弧线
...
- 读取 pitch.md（创作要求）、research.md、matrix.md、characters/、relationships.md、outline.md
- 严格按照专属文风和人物设定创作
...
1. **Phase 1** - 多个数据源并行请求
2. **Phase 2** - 全部事件并行调研
3. **Phase 4** - 入选事件并行分析（事件Matrix + 人物Matrix）
