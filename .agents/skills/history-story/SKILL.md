...
### Phase 4: 选题与创作要求（PitchEditor）

阅读 `agents/pitch-editor.md` 获取详细指令。

选题编辑在 Matrix 分析之后介入，基于分析结果做出选题决策：
...
4. **基于目标读者的代入角色定制专属文风**
5. **为每个入选事件提出创作要求**
6. 保存到 `./history/{MM}/{DD}/{event-slug}/pitch.md`（每个入选事件各自一份）

**选题逻辑**：不是选"最重要"的事件，而是选"最适合讲成故事"的事件。一个影响深远但缺乏人物张力的条约签署，不如一个影响较小但人物命运跌宕的小事件。

**目标读者推导逻辑**：
- E - Experience 为"英雄视角" → 目标读者渴望代入强者，文风偏向激昂有力
- E - Experience 为"旁观者视角" → 目标读者偏好审视与思考，文风偏向冷峻克制
- E - Experience 为"参与者视角" → 目标读者渴望沉浸体验，文风偏向细腻入微
...
整合所有素材设计故事大纲：
- 读取 pitch.md（创作要求+专属文风）、research.md、matrix.md、characters/、relationships.md
- 设计场景序列、叙事视角、情感弧线
...
- 读取 pitch.md（创作要求+专属文风）、research.md、matrix.md、characters/、relationships.md、outline.md
- 严格按照专属文风和人物设定创作
